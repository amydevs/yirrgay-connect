import { DialogButton, type DirectiveDescriptor, directivesPluginHooks } from '@mdxeditor/editor';
...
export const ScratchDirectiveDescriptor: DirectiveDescriptor<ScratchDirectiveNode> = {
    name: 'scratch',
    type: 'leafDirective',
    testNode(node) {
...
const ScratchButton = () => {
    ...
    return (
        <DialogButton
            tooltipTitle="Insert Scratch embed"
            submitButtonTitle="Insert Scratch"
            dialogInputPlaceholder="Paste the Scratch URL"
            buttonContent={ScratchIcon}
            onSubmit={(url) => {
                const scratchId = new URL(url).pathname.split('/').at(2);
                if (scratchId) {
                    insertDirective({
                        name: 'scratch',
                        type: 'leafDirective',

                        attributes: { id: scratchId },
                        children: []
                    } as LeafDirective)
                } else {
                    alert('Invalid Scratch URL')
                }
            }}
        />
    )
