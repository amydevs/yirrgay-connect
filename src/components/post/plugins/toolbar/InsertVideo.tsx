 ...
 */
export const InsertVideo = React.forwardRef<HTMLButtonElement, Record<string, never>>((_, forwardedRef) => {
  const openNewImageDialog = videoPluginHooks.usePublisher('openNewVideoDialog')
  const [readOnly] = corePluginHooks.useEmitterValues('readOnly')

  return (
    <RadixToolbar.Button ref={forwardedRef} disabled={readOnly} onClick={() => openNewImageDialog(true)}>
