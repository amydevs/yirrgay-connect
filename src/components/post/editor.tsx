...
import ScratchButton, {
  ScratchDirectiveDescriptor,
} from './directives/scratch';
...
>(
  (
    ...
    useEffect(() => {
      setMdxEditorClass(
        (theme.resolvedTheme ?? theme.theme) === 'dark' ? 'dark-theme' : '',
      );
    }, [theme]);
    ...
    return (
      <div
        ref={ref}
        className={cn('space-y-3 prose-lg dark:prose-invert', className)}
        {...props}
      >
        {!readOnly ? (
          <Input
            onChange={(evt) => {
              onChangeTitle?.(evt.target.value);
            }}
          ...
          <h1 className="px-3 mb-0">{valueTitle}</h1>
        )}
        ...
        <MDXEditor
          ...
          plugins={[
            ...
            directivesPlugin({
              directiveDescriptors: [ScratchDirectiveDescriptor],
            }),
            diffSourcePlugin({
              viewMode: 'rich-text',
              diffMarkdown: diffContent ?? valueContent,
            }),
            markdownShortcutPlugin(),
            htmlPlugin(),
          ]}
